	...
	public void exportExcel(HttpServletResponse response) throws IOException {
	     ...
	     //创建行对象  参数：索引（从0开始）
	       ...
	       for (int i = 0; i < cell.length; i++) {
			...
			// 给第0行第三列元素赋值
			cell[2].setCellValue("性别");
			// 给第0行第四列元素赋值
			cell[3].setCellValue("手机号");
			// 给第0行第五列元素赋值
			cell[4].setCellValue("部门");
			...
			for (int i = 0; i < list.size(); i++) {
				...
				cell2[0].setCellValue(employee.getEmp_id());
				cell2[1].setCellValue(employee.getEmp_name()); 
				cell2[2].setCellValue(employee.getEmp_sex());
				cell2[3].setCellValue(employee.getEmp_phone());
				cell2[4].setCellValue(employee.getEmp_bumen());
			}
	       ...
	       new EDownLoadUtils().download(os, response, riqi+"员工.xlsx");
